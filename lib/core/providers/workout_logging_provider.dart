...
final workoutLoggingServiceProvider = Provider((ref) => WorkoutLoggingService());

/// StreamProvider for workout logs (real-time)
final workoutLogsStreamProvider = StreamProvider<List<Map<String, dynamic>>>((ref) {
...
final todayWorkoutsStreamProvider = StreamProvider<List<Map<String, dynamic>>>((ref) {
  final service = ref.watch(workoutLoggingServiceProvider);
  return service.streamTodayWorkouts();
});

/// FutureProvider for workout stats
final workoutStatsProvider = FutureProvider.family<Map<String, dynamic>, int>((ref, days) async {
  final service = ref.watch(workoutLoggingServiceProvider);
  return service.getWorkoutStats(days: days);
});

/// FutureProvider for exercise progress
final exerciseProgressProvider = FutureProvider.family<List<Map<String, dynamic>>, Map<String, dynamic>>((ref, params) async {
  final service = ref.watch(workoutLoggingServiceProvider);
